...
function calculateFareLogic(start, end, uniform, tripType, mode) {
    ...
    if (totalMinutes < 120) {
        ...
        if (totalActualMinutes > 0) {
            // Proportional distribution for minimum charge
    ...
    return {
        totalMinutes, hours, mins, dayMinutes, nightMinutes,
...
function showError(message) {
    ...
    errorDiv.innerHTML = `<div class="alert alert-danger" role="alert"><i class="fas fa-exclamation-triangle me-2"></i><strong>Error:</strong> ${message}</div>`;
    ...
    resultDiv.innerHTML = `
        <div class="card ${statusColor}">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h4 class="card-title mb-0"><i class="${statusIcon} me-2"></i>${statusMessage}</h4>
...
async function bookTrip() {
    ...
    // 3. Compile all data for the server
    const tripData = {
        customerName, mobileNumber, startTime, endTime, tripType, mode, vehicleType, pickupLocation,
        dropLocation: mode === 'One Way' ? dropLocation : null, uniform, ...fareResults,
        startDate: start.toISOString(), endDate: end.toISOString()
    };
    
    // 4. Send data to server for booking/saving
